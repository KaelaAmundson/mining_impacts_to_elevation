...
mine_after_same_slope <- terrain(mine_after_same, opt='slope', unit='degrees')

plot(mine_after_same_slope)
...
mine_before_same_slope <- terrain(mine_before_same, opt='slope', unit='degrees')

mines_slope <- brick(mine_before_same_slope,mine_after_same_slope)

mines_slope[[3]] <- mines[[1]] - mines[[2]]

names(mines_slope) <- c('Pre_mining_slope','Post_mining_slope','DoS')
...
mine_before_same_slope %>%
  sphere_shade(texture = 'desert') %>%
  add_water(detect_water(mine_before_same_slope),color='desert') %>%
  add_shadow(ray_shade(mine_before_same_slope,zscale=2),0.5) %>%
  add_shadow(ambient_shade(mine_before_same_slope)) %>%
  plot_3d(mine_before_same_slope, heightmap=20,20, zscale=10,fov = 0,theta=135,zoom=0.75)
...
mine_after_same_slope %>%
  sphere_shade(texture = 'desert') %>%
  add_water(detect_water(mine_after_same_slope),color='desert') %>%
  add_shadow(ray_shade(mine_after_same_slope,zscale=2),0.5) %>%
  add_shadow(ambient_shade(mine_after_same_slope)) %>%
  plot_3d(mine_after_same_slope, heightmap=4, zscale=10,fov = 0,theta=135,zoom=0.75)
